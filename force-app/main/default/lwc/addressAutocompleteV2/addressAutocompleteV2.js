...
import getBusinessInternetProducts from "@salesforce/apex/ProductSelector.getBusinessInternetProducts";
...
export default class AddressAutocompleteV2 extends LightningElement {
	...
	@wire(getBusinessInternetProducts)
	wiredProducts({ error, data }) {
		if (data) {
			this.products = this.convertToComboboxOptions(data);
	...
	handleSubmit(event) {
		...
		fields.Manual_Product__c = this.selectedProduct;
