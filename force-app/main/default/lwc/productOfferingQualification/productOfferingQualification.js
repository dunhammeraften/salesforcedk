...
import MANUAL_PRODUCT_FIELD from "@salesforce/schema/Activation_line__c.Manual_Product__c";
...
export default class ProductOfferingQualification extends LightningElement {
	...
	subscribeToMessageChannel() {
		this.subscription = subscribe(this.messageContext, ONBOARDING_CHANNEL, (message) => {
	...
	async createActivationLine(line) {
		const fields = {
			...
			[MANUAL_PRODUCT_FIELD.fieldApiName]: String(line.Manual_Product__c || ""),
	...
	buildAddress({ Street_Name__c, Number__c, ZIP_Code__c, City__c, Floor__c, Door__c }) {
		// https://danmarksadresser.dk/om-adresser/saadan-gengives-en-adresse
		return `${Street_Name__c || ""} ${Number__c || ""}, ${Floor__c || ""} ${Door__c || ""}, ${ZIP_Code__c || ""} ${City__c || ""}`
